...
mod2 <- aov(GR ~ Site * Seedling_source * Soil_type, data = subset(group, Species == "P. asiatica"))
...
mod3 <- aov(GR ~ Site * Seedling_source * Soil_type, data = subset(group, Species == "T. repens"))
...
ggplot(p_value_long, aes(x = Effect, y = Proportion, fill = Significance)) +
  ...
  theme(panel.grid.major = element_blank(),
        ...
        axis.line = element_line(linetype = 1, color = "black", linewidth = 0.5),
