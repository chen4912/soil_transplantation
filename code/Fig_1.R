...
sample_size <- 8  # Sample size for each treatment combination
...
for (species in species_list) {
  data_subset <- subset(group, Species == species)# Raw data preparation
  for (i in 1:n_boot) {
    if (i %% 50 == 0) message("Processing iteration ", i, " for Species: ", species)# Progress Display
    # Stratified sampling
    ...
    tryCatch({
      ...
      # ========== Separate storage of ANOVA results ==========
...
# Convert the storage result to a data frame
...
ggplot(p_value_long, aes(x = Effect, y = Proportion, fill = Significance)) +
  ...
  theme_bw() +
  facet_grid(. ~ Species) +
  geom_hline(aes(yintercept = 70), linetype = "dashed", size = 1) +
