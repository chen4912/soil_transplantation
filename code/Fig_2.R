getwd()
setwd("../soil_transplantation-main/data/")
...
Type <- c("rhizosphere", "fungi","phyllophere")
plots <- list()
for (i in Type) {
  for (j in Species) {
    filtered_df <- df[df$Species == j & df$type == i, ]
...
##Make subtle offsets,factor=0.02 indicates that the amplitude of the random perturbation is 2% of the original data range
data3$elevation <- jitter(data3$elevation, factor=0.02)
ggplot(data3, aes(x = elevation, y = path, color = Species, group = Species)) +
  geom_point(size = 1.5, shape = 21) +  
  geom_smooth(method = "loess", se = F, size = 0.6, span = 1.1) + 
...
ggplot(data3, aes(x = elevation, y = AMF, color = Species, group = Species)) +
  geom_point(size = 1.5, shape = 21) + 
  geom_smooth(method = "loess", se = F, size = 0.6, span = 1.1) +  
  labs(y = "Community dissimilarity \n weighted Unifrac",
       x = "Elevation difference (m)", title = NULL) +
...
group$elevation <- jitter(group$elevation, factor=0.02)
