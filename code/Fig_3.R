...
head(fit_select)
# Perform model averaging for models with delta AIC < 2
best_est<-model.avg(fit_select,subset= delta <2, revised.var = TRUE)
summary(best_est)# Summarize the averaged model
sw(best_est)# Calculate and display the importance weights (SW) for each predictor
